...
function getLodop(): LodopSuccess | LodopError {
    ...
    if (!LODOP && CLodopJsState !== 'complete') {
        if (CLodopJsState == 'loading') {
            return {
                errorType: 'LOADING',
                errorMessage: '网页还没下载完毕，请稍等一下再操作.',
            };
        } else {
            return {
                errorType: 'LOADING',
                errorMessage: '没有加载CLodop的主js，请先调用loadCLodop过程.',
            };
        }
    }
    if (!LODOP) {
