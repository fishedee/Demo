		...
		let queryColumns = [
		 ...
		 const columns = [
	      ...
	      },
	      {
	        title: '角色',
	        dataIndex: 'role',
	        render: (val) =>roleOption[val],
		  },
		  {
			title:'是否可用',
			dataIndex:'isEnabled',
			render:(val)=>enabledOption[val],
		  },
	      {
	        title: '创建时间',
	      ...
	      },
	      {
	        ...
	        render: (val,data) => (
	          <Fragment>
	            <a onClick={this.mod.bind(this,data.userId)}>编辑</a>
	            <Divider type="vertical"/>
	            <a onClick={this.modPassword.bind(this,data.userId)}>修改密码</a>
	          </Fragment>
	        ),
	      },
	    ];
