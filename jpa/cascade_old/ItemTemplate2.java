...
public class ItemTemplate extends BaseEntityType {
    ...
    public static class Property{
        ...
        @OneToMany(fetch = FetchType.EAGER,mappedBy = "itemTemplatePropertyId")
        ...
        private void setOptions(OptionAddService optionAddService, List<ItemTemplateDTO.Option> newOptions){
            if( newOptions.size() < this.options.size()){
                throw new MyException(1,"不能删除原来"+this.name+"的选项数量",null);
            }
            int i = 0;
    ...
    @OneToMany(fetch = FetchType.EAGER,mappedBy = "itemTemplateId")
    @Fetch(FetchMode.SELECT)
    ...
    public void modProperties(List<ItemTemplateDTO.Property> newProperties){
        ...
        if( newProperties.size() < this.properties.size() ){
            throw new MyException(1,"不能少于原来的属性数量",null);
        }
        int i = 0;
